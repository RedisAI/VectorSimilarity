/*
 * fftw_guru_split_dft_r2c: real FFT using guru interface
 *
 * Arm Performance Libraries version 24.10
 * SPDX-FileCopyrightText: Copyright 2015-2024 Arm Limited and/or its affiliates

 * This example demonstrates performing FFTs on a 3-d (2x3x3) space of 4x3 matrices.
...
int main(void) {
	...
	printf("ARMPL example: FFT of a real sequence using fftw_plan_guru_split_dft_r2c/c2r\n");
	printf("----------------------------------------------------------------------------\n\n");
	...
	// Create the arrays for output of the r2c transform and input of the c2r transform
	double *yr = fftw_alloc_real(n0*n1_h*hm0*hm1*hm2);
	double *yi = fftw_alloc_real(n0*n1_h*hm0*hm1*hm2);

	// Create the FFTW plans
	fftw_plan forward  = fftw_plan_guru_split_dft_r2c(RANK, dims_r2c, HM_RANK, hm_dims_r2c,
	                                                  x, yr, yi, FFTW_ESTIMATE);
	fftw_plan backward = fftw_plan_guru_split_dft_c2r(RANK, dims_c2r, HM_RANK, hm_dims_c2r,
	                                                  yr, yi, x, FFTW_ESTIMATE);
