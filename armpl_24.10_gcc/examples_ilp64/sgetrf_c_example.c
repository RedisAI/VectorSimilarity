...
#define NMAX 8
#define NRHMAX 8
  int64_t lda, ldb, matrix_layout;
  int64_t i, info, j, n, nrhs;
  ...
  printf("ARMPL example: solution of linear equations using sgetrf/sgetrs\n");
  printf("---------------------------------------------------------------\n");
  printf("\n");
  ...
  /* Factorize A */
  info = LAPACKE_sgetrf(matrix_layout,n,n,a,lda,ipiv);

  printf("\n");
  if (info == 0)
    {
      /* Compute solution */
      info = LAPACKE_sgetrs(matrix_layout,'n',n,nrhs,a,lda,ipiv,b,ldb);
      /* Print solution */
