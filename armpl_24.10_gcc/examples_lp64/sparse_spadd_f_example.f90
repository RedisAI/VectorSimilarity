!   Double precision sparse matrix-matrix addition example
!
...
!   1. Set-up local CSR structure
      ...
      Real (Kind=armpl_r64)            :: alpha = 1.0, beta = 1.0
...
!   3c. Supply any hints that are about the SpAdd calculations to be performed with matrix A
      Call armpl_spmat_hint(armpl_mat_a, armpl_sparse_hint_spadd_operation, &
        armpl_sparse_operation_notrans, info)
      Call check_result(armpl_mat_a, info)

      Call armpl_spmat_hint(armpl_mat_a, armpl_sparse_hint_spadd_invocations, &
        armpl_sparse_invocations_single, info)
      Call check_result(armpl_mat_a, info)

!   3c. Supply any hints that are about the SpAdd calculations to be performed with matrix B
      Call armpl_spmat_hint(armpl_mat_b, armpl_sparse_hint_spadd_operation, &
        armpl_sparse_operation_notrans, info)
      Call check_result(armpl_mat_b, info)

      Call armpl_spmat_hint(armpl_mat_b, armpl_sparse_hint_spadd_invocations, &
        armpl_sparse_invocations_single, info)
      Call check_result(armpl_mat_b, info)

!   4. Optimize the matrices based on the hints previously supplied
      Call armpl_spadd_optimize(armpl_sparse_operation_notrans, &
        armpl_sparse_operation_notrans, armpl_sparse_scalar_one, armpl_mat_a, &
        armpl_sparse_scalar_one, armpl_mat_b, armpl_mat_c, info)
      Call check_result(armpl_mat_c, info)

!   5a. Do SpAdd
      Call armpl_spadd_exec(armpl_sparse_operation_notrans, &
        armpl_sparse_operation_notrans, alpha, armpl_mat_a, beta, armpl_mat_b, &
        armpl_mat_c, info)
      Call check_result(armpl_mat_c, info)
