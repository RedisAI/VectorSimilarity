...
void perform_fft(double complex *in_data, double *out, int n) {
...
// given m, n, and nnz
void generate_csr_format(int m, int n, int nnz, int *col_indx,
                         int *row_ptr) {
	int i, k, row, col;
	// Tracking position (row, col)
	int **filled = (int **) malloc((m + 1) * sizeof(int *));
	for (i = 0; i <= m; i++) {
		filled[i] = (int *) calloc((n + 1), sizeof(int));
	}
...
int main() {
	...
	int *col_indx = (int *) malloc(nnz * sizeof(int));
