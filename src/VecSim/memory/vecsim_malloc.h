...
static inline void *vecsim_malloc(size_t n) {
...
static inline void *vecsim_realloc(void *p, size_t n) {
	return RedisModule_Realloc(p, n);
}
static inline void vecsim_free(void *p) {
	RedisModule_Free(p);
}
static inline char *vecsim_strdup(const char *s) {
