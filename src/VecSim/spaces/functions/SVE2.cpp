...
#include "VecSim/spaces/IP/IP_SVE2_FP64.h"
#include "VecSim/spaces/L2/L2_SVE2_FP64.h"
namespace spaces {
...
dist_func_t<double> Choose_FP64_IP_implementation_SVE2(size_t dim) {
    ...
    CHOOSE_RUNTIME_IMPLEMENTATION(ret_dist_func, dim, 4 * svcntd(), FP64_InnerProductSIMD_SVE2);
    return ret_dist_func;
}
...
dist_func_t<double> Choose_FP64_L2_implementation_SVE2(size_t dim) {
    dist_func_t<double> ret_dist_func;
    CHOOSE_RUNTIME_IMPLEMENTATION(ret_dist_func, dim, 4 * svcntd(), FP64_L2SqrSIMD_SVE2);
    return ret_dist_func;
}
