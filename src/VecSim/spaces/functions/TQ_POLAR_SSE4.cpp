...
int TQ_PackedResidualSignDotSIMD16_SSE4(const uint8_t *lhs, const uint8_t *rhs,
                                        size_t projections) {
    const size_t full_bytes = projections / 8;
    ...
    for (; idx < simd_end; idx += 16) {
        const __m128i diff =
            _mm_xor_si128(_mm_loadu_si128(reinterpret_cast<const __m128i *>(lhs + idx)),
                          _mm_loadu_si128(reinterpret_cast<const __m128i *>(rhs + idx)));
        const __m128i bit_counts = PopcountBytes(diff);
        const __m128i partial = _mm_sad_epu8(bit_counts, zero);
        diff_count_total += static_cast<uint64_t>(_mm_cvtsi128_si64(partial));
        diff_count_total += static_cast<uint64_t>(_mm_extract_epi64(partial, 1));
    }
...
template <unsigned char residual>
float TQ_SymmetricPolarSIMD4_SSE4(const float *lhs_radii, const uint8_t *lhs_angles,
    ...
    alignas(16) uint8_t deltas[4];
    alignas(16) float lut_values[4];

    size_t idx = 0;
    for (; idx < simd_end; idx += 4) {
        uint32_t lhs_packed = 0;
        uint32_t rhs_packed = 0;
        std::memcpy(&lhs_packed, lhs_angles + idx, sizeof(lhs_packed));
        std::memcpy(&rhs_packed, rhs_angles + idx, sizeof(rhs_packed));

        const __m128i lhs_vec = _mm_cvtsi32_si128(static_cast<int>(lhs_packed));
        const __m128i rhs_vec = _mm_cvtsi32_si128(static_cast<int>(rhs_packed));
        const __m128i delta_vec = _mm_and_si128(_mm_sub_epi8(lhs_vec, rhs_vec), mask_vec);

        const uint32_t delta_packed = static_cast<uint32_t>(_mm_cvtsi128_si32(delta_vec));
        std::memcpy(deltas, &delta_packed, sizeof(delta_packed));
        for (size_t lane = 0; lane < 4; ++lane) {
            lut_values[lane] = delta_cos_lut[deltas[lane]];
        }

        const __m128 lhs_vec_f = _mm_loadu_ps(lhs_radii + idx);
        const __m128 rhs_vec_f = _mm_loadu_ps(rhs_radii + idx);
        const __m128 lut_vec = _mm_load_ps(lut_values);
        acc = _mm_add_ps(acc, _mm_mul_ps(_mm_mul_ps(lhs_vec_f, rhs_vec_f), lut_vec));
    }

    alignas(16) float partials[4];
    _mm_store_ps(partials, acc);
    float sum = partials[0] + partials[1] + partials[2] + partials[3];
...
Choose_TQ_PackedResidualSignDot_implementation_SSE4(size_t projections) {
    const size_t full_bytes = projections / 8;
    tq_packed_residual_sign_dot_func_t ret_func;
    CHOOSE_IMPLEMENTATION(ret_func, full_bytes, 16, TQ_PackedResidualSignDotSIMD16_SSE4);
    return ret_func;
}

tq_symmetric_polar_func_t Choose_TQ_SymmetricPolar_implementation_SSE4(size_t pairs) {
    tq_symmetric_polar_func_t ret_func;
    CHOOSE_IMPLEMENTATION(ret_func, pairs, 4, TQ_SymmetricPolarSIMD4_SSE4);
    return ret_func;
}
