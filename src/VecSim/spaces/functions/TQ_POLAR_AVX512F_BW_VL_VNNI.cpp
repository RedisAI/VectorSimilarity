...
inline __m512i PopcountBytes(__m512i bytes) {
    ...
    const __m512i lookup = _mm512_load_si512(reinterpret_cast<const void *>(lookup_bytes));
...
float TQ_SymmetricPolarSIMD16_AVX512F_BW_VL_VNNI(const float *lhs_radii, const uint8_t *lhs_angles,
    ...
    for (; idx < simd_end; idx += 16) {
        const __m128i lhs_vec =
            _mm_loadu_si128(reinterpret_cast<const __m128i *>(lhs_angles + idx));
        const __m128i rhs_vec =
            _mm_loadu_si128(reinterpret_cast<const __m128i *>(rhs_angles + idx));
