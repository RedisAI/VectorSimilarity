...
dist_func_t<float> IP_INT4_GetDistFunc(size_t dim, unsigned char *alignment, const void *arch_opt) {
    unsigned char dummy_alignment;
    if (alignment == nullptr) {
        alignment = &dummy_alignment;
    }
    ...
    dist_func_t<float> ret_dist_func = INT4_InnerProduct;
    return ret_dist_func;
}

dist_func_t<float> Cosine_INT4_GetDistFunc(size_t dim, unsigned char *alignment,
                                           const void *arch_opt) {
    unsigned char dummy_alignment;
    if (alignment == nullptr) {
        alignment = &dummy_alignment;
    ...
    // For now, only naive implementation - SIMD optimizations can be added later
    dist_func_t<float> ret_dist_func = INT4_Cosine;
    return ret_dist_func;
}
