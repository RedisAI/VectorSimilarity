...
class BF16Encoder : public EncoderInterface<float, bf16, float>, VecsimBaseObject {
    ...
    public:
        BF16Encoder(std::shared_ptr<VecSimAllocator> allocator, size_t dim): VecsimBaseObject(allocator), dim(dim) {
...
bf16_to_fp32_encoder_t Get_BF16_to_FP32_Encoder(size_t dim, const Arch_Optimization arch_opt, bool big_endian);
