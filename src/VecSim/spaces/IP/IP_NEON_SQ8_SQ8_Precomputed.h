...
static inline void SQ8_SQ8_Precomputed_InnerProductStep_NEON(const uint8_t *&pVec1,
                                                             const uint8_t *&pVec2,
                                                             float32x4_t &dot_sum) {
...
float SQ8_SQ8_Precomputed_InnerProductSIMD16_NEON_IMP(const void *pVec1v, const void *pVec2v,
                                                      size_t dimension) {
    ...
    for (size_t i = 0; i < num_of_chunks; i++) {
        SQ8_SQ8_Precomputed_InnerProductStep_NEON(pVec1, pVec2, dot_sum0);
        SQ8_SQ8_Precomputed_InnerProductStep_NEON(pVec1, pVec2, dot_sum1);
        SQ8_SQ8_Precomputed_InnerProductStep_NEON(pVec1, pVec2, dot_sum2);
        SQ8_SQ8_Precomputed_InnerProductStep_NEON(pVec1, pVec2, dot_sum3);
    }
    ...
    return delta1 * delta2 * dot_product + delta1 * min2 * sum1 + delta2 * min1 * sum2 +
...
template <unsigned char residual> // 0..15
float SQ8_SQ8_Precomputed_InnerProductSIMD16_NEON(const void *pVec1v, const void *pVec2v,
                                                  size_t dimension) {
    return 1.0f -
           SQ8_SQ8_Precomputed_InnerProductSIMD16_NEON_IMP<residual>(pVec1v, pVec2v, dimension);
}

// SQ8-to-SQ8 Cosine distance function with precomputed sum/norm
// Returns 1 - inner_product (assumes vectors are pre-normalized)
template <unsigned char residual> // 0..15
float SQ8_SQ8_Precomputed_CosineSIMD16_NEON(const void *pVec1v, const void *pVec2v,
                                            size_t dimension) {
    return 1.0f -
           SQ8_SQ8_Precomputed_InnerProductSIMD16_NEON_IMP<residual>(pVec1v, pVec2v, dimension);
}
