...
static inline void InnerProduct_step(const float16_t *&vec1, const float16_t *&vec2,
    ...
    acc = vfmaq_f16(acc, v1, v2); // TODO: is this the correct order?
}
...
float FP16_InnerProduct_NEON_HP(const void *pVect1v, const void *pVect2v, size_t dimension) {
    ...
    if constexpr (residual % 8) {
        ...
        // Multiply and accumulate
        acc = vfmaq_f16(acc, masked_v1, masked_v2);
    ...
    if constexpr (residual >= 24)
        InnerProduct_step(vec1, vec2, acc);
    ...
    float32x4_t sum_f32 = vcvt_f32_f16(vget_low_f16(acc));
    sum_f32 = vaddq_f32(sum_f32, vcvt_f32_f16(vget_high_f16(acc)));
