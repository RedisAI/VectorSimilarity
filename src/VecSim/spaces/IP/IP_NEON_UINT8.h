...
template <unsigned char residual> // 0..63
struct UINT8_IPChunkKernel_NEON {
    ...
    static uint32_t rest(const uint8_t *pVect1, const uint8_t *pVect2, size_t dimension) {
        return UINT8_InnerProductFullChunk_NEON(pVect1, pVect2, dimension);
    }
};
...
float UINT8_InnerProductSIMD16_NEON_Chunked(const void *pVect1v, const void *pVect2v,
                                            size_t dimension) {
    const auto ip =
        static_cast<int64_t>(spaces::uint8_chunked_total<UINT8_IPChunkKernel_NEON<residual>>(
...
float UINT8_CosineSIMD_NEON_Chunked(const void *pVect1v, const void *pVect2v, size_t dimension) {
    float ip = static_cast<float>(spaces::uint8_chunked_total<UINT8_IPChunkKernel_NEON<residual>>(
        pVect1v, pVect2v, dimension));
    const float norm_v1 = load_unaligned<float>(static_cast<const uint8_t *>(pVect1v) + dimension);
    const float norm_v2 = load_unaligned<float>(static_cast<const uint8_t *>(pVect2v) + dimension);
    return 1.0f - ip / (norm_v1 * norm_v2);
