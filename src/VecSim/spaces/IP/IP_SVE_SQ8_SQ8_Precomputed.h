 ...
 *
 * This version uses precomputed sum and norm stored in the vector data,
 ...
 *
 * Since sum = Σv[i] is precomputed, we only need to compute the dot product Σ(v1[i]*v2[i]).
 *
...
static inline void SQ8_SQ8_Precomputed_InnerProductStep_SVE(const uint8_t *pVec1,
                                                            const uint8_t *pVec2, size_t &offset,
                                                            svuint32_t &dot_sum,
                                                            const size_t chunk) {
...
float SQ8_SQ8_Precomputed_InnerProductSIMD_SVE_IMP(const void *pVec1v, const void *pVec2v,
                                                   size_t dimension) {
    ...
    const float sum1 = params1[2]; // Precomputed sum of uint8 elements
    ...
    const float sum2 = params2[2]; // Precomputed sum of uint8 elements
    ...
    for (size_t i = 0; i < number_of_chunks; i++) {
        SQ8_SQ8_Precomputed_InnerProductStep_SVE(pVec1, pVec2, offset, dot_sum0, vl);
        SQ8_SQ8_Precomputed_InnerProductStep_SVE(pVec1, pVec2, offset, dot_sum1, vl);
        SQ8_SQ8_Precomputed_InnerProductStep_SVE(pVec1, pVec2, offset, dot_sum2, vl);
        SQ8_SQ8_Precomputed_InnerProductStep_SVE(pVec1, pVec2, offset, dot_sum3, vl);
    }
    ...
    return delta1 * delta2 * static_cast<float>(dot_product) + delta1 * min2 * sum1 +
           delta2 * min1 * sum2 + static_cast<float>(dimension) * min1 * min2;
}

// SQ8-to-SQ8 Inner Product distance function with precomputed sum/norm
// Returns 1 - inner_product (distance form)
template <bool partial_chunk, unsigned char additional_steps>
float SQ8_SQ8_Precomputed_InnerProductSIMD_SVE(const void *pVec1v, const void *pVec2v,
                                               size_t dimension) {
    return 1.0f - SQ8_SQ8_Precomputed_InnerProductSIMD_SVE_IMP<partial_chunk, additional_steps>(
                      pVec1v, pVec2v, dimension);
}

// SQ8-to-SQ8 Cosine distance function with precomputed sum/norm
// Returns 1 - inner_product (assumes vectors are pre-normalized)
template <bool partial_chunk, unsigned char additional_steps>
float SQ8_SQ8_Precomputed_CosineSIMD_SVE(const void *pVec1v, const void *pVec2v, size_t dimension) {
    return 1.0f - SQ8_SQ8_Precomputed_InnerProductSIMD_SVE_IMP<partial_chunk, additional_steps>(
                      pVec1v, pVec2v, dimension);
}
