...
static inline void SQ8_SQ8_Precomputed_InnerProductStep64(const uint8_t *pVec1,
                                                          const uint8_t *pVec2, __m512i &dot_acc0,
                                                          __m512i &dot_acc1) {
...
static inline void SQ8_SQ8_Precomputed_InnerProductStep32(const uint8_t *pVec1,
                                                          const uint8_t *pVec2, __m512i &dot_acc) {
...
float SQ8_SQ8_Precomputed_InnerProductImp(const void *pVec1v, const void *pVec2v,
                                          size_t dimension) {
    ...
    if constexpr (residual > 0) {
        if constexpr (residual < 32) {
            ...
            // Exactly 32 elements
            SQ8_SQ8_Precomputed_InnerProductStep32(pVec1, pVec2, dot_acc0);
        } else {
    ...
    while (pVec1 < pEnd1) {
        SQ8_SQ8_Precomputed_InnerProductStep64(pVec1, pVec2, dot_acc0, dot_acc1);
        pVec1 += 64;
        pVec2 += 64;
    }
    ...
    float result = delta1 * delta2 * static_cast<float>(dot_product) + delta1 * min2 * sum1 +
...
template <unsigned char residual> // 0..63
float SQ8_SQ8_Precomputed_InnerProductSIMD64_AVX512F_BW_VL_VNNI(const void *pVec1v,
                                                                const void *pVec2v,
                                                                size_t dimension) {
    return 1.0f - SQ8_SQ8_Precomputed_InnerProductImp<residual>(pVec1v, pVec2v, dimension);
}

// SQ8-to-SQ8 Cosine distance function with precomputed sum/norm
// Returns 1 - (inner_product)
template <unsigned char residual> // 0..63
float SQ8_SQ8_Precomputed_CosineSIMD64_AVX512F_BW_VL_VNNI(const void *pVec1v, const void *pVec2v,
                                                          size_t dimension) {
