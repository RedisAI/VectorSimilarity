...
float FP16_InnerProductSIMD16_AVX512BW_VL(const void *pVect1v, const void *pVect2v, size_t dimension) {
   ...
   if (residual) {
       ...
       // Convert the first 16 half-floats into floats and store them 512 bits register,
       // where the floats in the positions corresponding to residuals are zeros.
       auto v1 = _mm512_maskz_loadu_ps(_mm512_cvtph_ps(_mm256_loadu_epi16(pVect1)),
                                 residuals_mask);
       auto v2 = _mm512_maskz_loadu_ps(_mm512_setzero_ps(),
                                 _mm512_cvtph_ps(_mm256_loadu_epi16(pVect2)),
                                 residuals_mask);
