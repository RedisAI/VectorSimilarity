...
inline void L2Sqr_Step(const float16_t *&vec1, const float16_t *&vec2, float32x4_t &acc_lo,
                       float32x4_t &acc_hi) {
    ...
    // Widen both halves to single precision, then subtract and accumulate in fp32.
    float32x4_t d_lo = vsubq_f32(vcvt_f32_f16(vget_low_f16(v1)), vcvt_f32_f16(vget_low_f16(v2)));
    float32x4_t d_hi = vsubq_f32(vcvt_high_f32_f16(v1), vcvt_high_f32_f16(v2));
...
float FP16_L2Sqr_NEON_HP(const void *pVect1v, const void *pVect2v, size_t dimension) {
    ...
    if constexpr (residual % 8) {
        ...
        // Widen, subtract and accumulate in fp32; the masked lanes contribute zero.
        float32x4_t d_lo =
            vsubq_f32(vcvt_f32_f16(vget_low_f16(masked_v1)), vcvt_f32_f16(vget_low_f16(masked_v2)));
        float32x4_t d_hi = vsubq_f32(vcvt_high_f32_f16(masked_v1), vcvt_high_f32_f16(masked_v2));
        acc1_lo = vfmaq_f32(acc1_lo, d_lo, d_lo);
        acc1_hi = vfmaq_f32(acc1_hi, d_hi, d_hi);
    ...
    if constexpr (residual >= 8)
        L2Sqr_Step(vec1, vec2, acc2_lo, acc2_hi);
    if constexpr (residual >= 16)
        L2Sqr_Step(vec1, vec2, acc3_lo, acc3_hi);
    if constexpr (residual >= 24)
        L2Sqr_Step(vec1, vec2, acc4_lo, acc4_hi);

    // Process the rest of the vectors (the full chunks part)
    while (vec1 < v1End) {
        // TODO: use `vld1q_f16_x4` for quad-loading?
        L2Sqr_Step(vec1, vec2, acc1_lo, acc1_hi);
        L2Sqr_Step(vec1, vec2, acc2_lo, acc2_hi);
        L2Sqr_Step(vec1, vec2, acc3_lo, acc3_hi);
        L2Sqr_Step(vec1, vec2, acc4_lo, acc4_hi);
    }
