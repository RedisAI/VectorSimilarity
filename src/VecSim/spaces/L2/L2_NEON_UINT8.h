...
UINT8_L2SqrImp_NEON(const void *pVect1v, const void *pVect2v, size_t dimension) {
    ...
    // int32 this wrapped negative from dimension 33,026. Exact for up to
...
float UINT8_L2SqrSIMD16_NEON_Chunked(const void *pVect1v, const void *pVect2v, size_t dimension) {
    ...
    uint64_t total = UINT8_L2SqrImp_NEON<residual>(pVect1, pVect2, first);
    pVect1 += first;
    pVect2 += first;
    ...
    while (remaining) {
        ...
        total += UINT8_L2SqrFullChunk_NEON(pVect1, pVect2, step);
        pVect1 += step;
        pVect2 += step;
        remaining -= step;
    }
