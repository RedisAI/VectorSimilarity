...
class OLDHNSWMulti_BatchIterator : public OLDHNSW_BatchIterator<DataType, DistType> {
private:
...
public:
    OLDHNSWMulti_BatchIterator(void *query_vector, const OLDHNSWIndex<DataType, DistType> *index,
                               VecSimQueryParams *queryParams,
                               std::shared_ptr<VecSimAllocator> allocator)
        : OLDHNSW_BatchIterator<DataType, DistType>(query_vector, index, queryParams, allocator),
          returned(this->index->indexSize(), this->allocator) {}

    ~OLDHNSWMulti_BatchIterator() override {}
...
template <typename DataType, typename DistType>
VecSimQueryResult_List OLDHNSWMulti_BatchIterator<DataType, DistType>::prepareResults(
...
template <typename DataType, typename DistType>
void OLDHNSWMulti_BatchIterator<DataType, DistType>::fillFromExtras(
...
template <typename DataType, typename DistType>
void OLDHNSWMulti_BatchIterator<DataType, DistType>::updateHeaps(
...
template <typename DataType, typename DistType>
void OLDHNSWMulti_BatchIterator<DataType, DistType>::reset() {
    this->returned.clear();
    OLDHNSW_BatchIterator<DataType, DistType>::reset();
}
