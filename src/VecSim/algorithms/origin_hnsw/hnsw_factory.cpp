...
inline VecSimIndex *NewIndex_ChooseMultiOrSingle(const HNSWParams *params,
    ...
    if (params->multi)
        return new (allocator) OLDHNSWIndex_Multi<DataType, DistType>(params, allocator);
    else
        return new (allocator) OLDHNSWIndex_Single<DataType, DistType>(params, allocator);
}
...
inline size_t EstimateInitialSize_ChooseMultiOrSingle(bool is_multi) {
    ...
    if (is_multi)
        return sizeof(OLDHNSWIndex_Multi<DataType, DistType>);
    else
        return sizeof(OLDHNSWIndex_Single<DataType, DistType>);
}
...
inline VecSimIndex *NewIndex_ChooseMultiOrSingle(std::ifstream &input, const HNSWParams *params,
    ...
    OLDHNSWIndex<DataType, DistType> *index = nullptr;
    // check if single and call the ctor that loads index information from file.
    if (params->multi)
        index = new (allocator)
            OLDHNSWIndex_Multi<DataType, DistType>(input, params, allocator, version);
    else
        index = new (allocator)
            OLDHNSWIndex_Single<DataType, DistType>(input, params, allocator, version);
