...
#define OLDHNSW_INVALID_ID    UINT_MAX
#define OLDHNSW_INVALID_LEVEL SIZE_MAX
...
class OLDHNSWIndex : public VecSimIndexAbstract<DistType>,
...
#ifdef BUILD_TESTS
#include "VecSim/algorithms/origin_hnsw/hnsw_base_tests_friends.h"

#include "hnsw_serializer_declarations.h"
#endif

protected:
    OLDHNSWIndex() = delete;                     // default constructor is disabled.
    OLDHNSWIndex(const OLDHNSWIndex &) = delete; // default (shallow) copy constructor is disabled.
...
public:
    OLDHNSWIndex(const HNSWParams *params, std::shared_ptr<VecSimAllocator> allocator,
                 size_t random_seed = 100, size_t initial_pool_size = 1);
    virtual ~OLDHNSWIndex();
...
template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::setEf(size_t ef) {
    ef_ = ef;
}

template <typename DataType, typename DistType>
size_t OLDHNSWIndex<DataType, DistType>::getEf() const {
    return ef_;
}

template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::setEpsilon(double epsilon) {
    epsilon_ = epsilon;
}

template <typename DataType, typename DistType>
double OLDHNSWIndex<DataType, DistType>::getEpsilon() const {
    return epsilon_;
}

template <typename DataType, typename DistType>
size_t OLDHNSWIndex<DataType, DistType>::indexSize() const {
    return cur_element_count - num_marked_deleted;
}

template <typename DataType, typename DistType>
size_t OLDHNSWIndex<DataType, DistType>::getIndexCapacity() const {
    return max_elements_;
}

template <typename DataType, typename DistType>
size_t OLDHNSWIndex<DataType, DistType>::getEfConstruction() const {
    return ef_construction_;
}

template <typename DataType, typename DistType>
size_t OLDHNSWIndex<DataType, DistType>::getM() const {
    return M_;
}

template <typename DataType, typename DistType>
size_t OLDHNSWIndex<DataType, DistType>::getMaxLevel() const {
    return maxlevel_;
}

template <typename DataType, typename DistType>
labelType OLDHNSWIndex<DataType, DistType>::getEntryPointLabel() const {
    if (entrypoint_node_ != OLDHNSW_INVALID_ID)
        return getExternalLabel(entrypoint_node_);
    return SIZE_MAX;
}

template <typename DataType, typename DistType>
labelType OLDHNSWIndex<DataType, DistType>::getExternalLabel(idType internal_id) const {
...
template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::setExternalLabel(idType internal_id, labelType label) {
    memcpy((data_level0_memory_ + internal_id * size_data_per_element_ + label_offset_), &label,
           sizeof(labelType));
}

template <typename DataType, typename DistType>
labelType *OLDHNSWIndex<DataType, DistType>::getExternalLabelPtr(idType internal_id) const {
    return (labelType *)(data_level0_memory_ + internal_id * size_data_per_element_ +
                         label_offset_);
}

template <typename DataType, typename DistType>
char *OLDHNSWIndex<DataType, DistType>::getDataByInternalId(idType internal_id) const {
    return (data_level0_memory_ + internal_id * size_data_per_element_ + offsetData_);
}

template <typename DataType, typename DistType>
size_t OLDHNSWIndex<DataType, DistType>::getRandomLevel(double reverse_size) {
...
OLDHNSWIndex<DataType, DistType>::getIncomingEdgesPtr(idType internal_id, size_t level) const {
    if (level == 0) {
...
template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::setIncomingEdgesPtr(idType internal_id, size_t level,
                                                           void *edges_ptr) {
    if (level == 0) {
...
template <typename DataType, typename DistType>
elementFlags *OLDHNSWIndex<DataType, DistType>::get_flags(idType internal_id) const {
...
template <typename DataType, typename DistType>
idType *OLDHNSWIndex<DataType, DistType>::get_linklist0(idType internal_id) const {
...
template <typename DataType, typename DistType>
idType *OLDHNSWIndex<DataType, DistType>::get_linklist(idType internal_id, size_t level) const {
...
template <typename DataType, typename DistType>
idType *OLDHNSWIndex<DataType, DistType>::get_linklist_at_level(idType internal_id,
                                                                size_t level) const {
    return level == 0 ? get_linklist0(internal_id) : get_linklist(internal_id, level);
}

template <typename DataType, typename DistType>
linkListSize OLDHNSWIndex<DataType, DistType>::getListCount(const idType *list) const {
    return *(((linkListSize *)list) - 1);
}

template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::setListCount(idType *list, const linkListSize size) {
    *(((linkListSize *)list) - 1) = size;
}

template <typename DataType, typename DistType>
idType OLDHNSWIndex<DataType, DistType>::getEntryPointId() const {
    return entrypoint_node_;
}

template <typename DataType, typename DistType>
VisitedNodesHandler *OLDHNSWIndex<DataType, DistType>::getVisitedList() const {
    return visited_nodes_handler.get();
}

template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::markDeletedInternal(idType internalId) {
...
template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::unmarkDeletedInternal(idType internalId) {
    assert(internalId < this->cur_element_count);
    if (isMarkedDeleted(internalId)) {
        elementFlags *flags = get_flags(internalId);
        *flags &= ~DELETE_MARK;
...
template <typename DataType, typename DistType>
bool OLDHNSWIndex<DataType, DistType>::isMarkedDeleted(idType internalId) const {
    elementFlags *flags = get_flags(internalId);
 ...
 */
template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::removeExtraLinks(
...
template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::emplaceToHeap(
...
DistType OLDHNSWIndex<DataType, DistType>::processCandidate(
...
template <typename DataType, typename DistType>
template <bool has_marked_deleted>
void OLDHNSWIndex<DataType, DistType>::processCandidate_RangeSearch(
...
OLDHNSWIndex<DataType, DistType>::searchLayer(idType ep_id, const void *data_point, size_t layer,
                                              size_t ef) const {
...
template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::getNeighborsByHeuristic2(
...
template <typename DataType, typename DistType>
idType OLDHNSWIndex<DataType, DistType>::mutuallyConnectNewElement(
...
template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::repairConnectionsForDeletion(
...
template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::replaceEntryPoint() {
    ...
    while (old_entry == entrypoint_node_) {
        ...
        if (old_entry == entrypoint_node_) {
            maxlevel_--;
            if ((int)maxlevel_ < 0) {
                maxlevel_ = OLDHNSW_INVALID_LEVEL;
                entrypoint_node_ = OLDHNSW_INVALID_ID;
            }
        }
    }
}
...
template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::SwapLastIdWithDeletedId(idType element_internal_id) {
    // swap label
    ...
    element_levels_[cur_element_count] = OLDHNSW_INVALID_LEVEL;
...
template <typename DataType, typename DistType>
template <bool with_timeout>
void OLDHNSWIndex<DataType, DistType>::greedySearchLevel(const void *vector_data, size_t level,
...
template <typename DataType, typename DistType>
OLDHNSWIndex<DataType, DistType>::OLDHNSWIndex(const HNSWParams *params,
...
#endif
{
    ...
    if (linkLists_ == nullptr)
        throw std::runtime_error("Not enough memory: OLDHNSWIndex failed to allocate linklists");
...
template <typename DataType, typename DistType>
OLDHNSWIndex<DataType, DistType>::~OLDHNSWIndex() {
 ...
 */
template <typename DataType, typename DistType>
void OLDHNSWIndex<DataType, DistType>::resizeIndex(size_t new_max_elements) {
...
template <typename DataType, typename DistType>
int OLDHNSWIndex<DataType, DistType>::removeVector(const idType element_internal_id) {
...
template <typename DataType, typename DistType>
int OLDHNSWIndex<DataType, DistType>::appendVector(const void *vector_data, const labelType label) {
...
template <typename DataType, typename DistType>
idType OLDHNSWIndex<DataType, DistType>::searchBottomLayerEP(const void *query_data,
                                                             void *timeoutCtx,
                                                             VecSimQueryResult_Code *rc) const {
    *rc = VecSim_QueryResult_OK;
    ...
    for (size_t level = maxlevel_; level > 0 && currObj != OLDHNSW_INVALID_ID; level--) {
...
template <typename DataType, typename DistType>
template <bool has_marked_deleted>
candidatesLabelsMaxHeap<DistType> *OLDHNSWIndex<DataType, DistType>::searchBottomLayer_WithTimeout(
...
template <typename DataType, typename DistType>
VecSimQueryResult_List OLDHNSWIndex<DataType, DistType>::topKQuery(const void *query_data, size_t k,
...
template <typename DataType, typename DistType>
template <bool has_marked_deleted>
VecSimQueryResult *OLDHNSWIndex<DataType, DistType>::searchRangeBottomLayer_WithTimeout(
...
OLDHNSWIndex<DataType, DistType>::rangeQuery(const void *query_data, double radius,
                                             VecSimQueryParams *queryParams) {
    VecSimQueryResult_List rl = {0};
...
template <typename DataType, typename DistType>
VecSimIndexInfo OLDHNSWIndex<DataType, DistType>::info() const {
...
template <typename DataType, typename DistType>
VecSimInfoIterator *OLDHNSWIndex<DataType, DistType>::infoIterator() const {
...
template <typename DataType, typename DistType>
bool OLDHNSWIndex<DataType, DistType>::preferAdHocSearch(size_t subsetSize, size_t k,
                                                         bool initial_check) {
