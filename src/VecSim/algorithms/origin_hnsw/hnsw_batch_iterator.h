...
class OLDHNSW_BatchIterator : public VecSimBatchIterator {
protected:
    const OLDHNSWIndex<DataType, DistType> *index;
...
public:
    OLDHNSW_BatchIterator(void *query_vector, const OLDHNSWIndex<DataType, DistType> *index,
    ...
    ~OLDHNSW_BatchIterator() override {}
};

/******************** Ctor / Dtor **************/

template <typename DataType, typename DistType>
OLDHNSW_BatchIterator<DataType, DistType>::OLDHNSW_BatchIterator(
    void *query_vector, const OLDHNSWIndex<DataType, DistType> *index,
...
template <typename DataType, typename DistType>
template <bool has_marked_deleted>
VecSimQueryResult_Code OLDHNSW_BatchIterator<DataType, DistType>::scanGraphInternal(
...
OLDHNSW_BatchIterator<DataType, DistType>::scanGraph(VecSimQueryResult_Code *rc) {

    candidatesLabelsMaxHeap<DistType> *top_candidates = this->index->getNewMaxPriorityQueue();
    if (this->entry_point == OLDHNSW_INVALID_ID) {
        this->depleted = true;
...
OLDHNSW_BatchIterator<DataType, DistType>::getNextResults(size_t n_res,
...
template <typename DataType, typename DistType>
bool OLDHNSW_BatchIterator<DataType, DistType>::isDepleted() {
    return this->depleted && this->top_candidates_extras.empty();
}

template <typename DataType, typename DistType>
void OLDHNSW_BatchIterator<DataType, DistType>::reset() {
