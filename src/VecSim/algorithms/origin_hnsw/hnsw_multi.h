...
class OLDHNSWIndex_Multi : public OLDHNSWIndex<DataType, DistType> {
private:
    vecsim_stl::unordered_map<labelType, vecsim_stl::vector<idType>> label_lookup_;

#ifdef BUILD_TESTS
#include "VecSim/algorithms/origin_hnsw/hnsw_multi_tests_friends.h"
#endif
...
public:
    OLDHNSWIndex_Multi(const HNSWParams *params, std::shared_ptr<VecSimAllocator> allocator,
    ...
    OLDHNSWIndex_Multi(std::ifstream &input, const HNSWParams *params,
...
template <typename DataType, typename DistType>
size_t OLDHNSWIndex_Multi<DataType, DistType>::indexLabelCount() const {
    return label_lookup_.size();
}

template <typename DataType, typename DistType>
double OLDHNSWIndex_Multi<DataType, DistType>::getDistanceFrom(labelType label,
...
template <typename DataType, typename DistType>
void OLDHNSWIndex_Multi<DataType, DistType>::replaceIdOfLabel(labelType label, idType new_id,
                                                              idType old_id) {
...
template <typename DataType, typename DistType>
void OLDHNSWIndex_Multi<DataType, DistType>::resizeLabelLookup(size_t new_max_elements) {
...
template <typename DataType, typename DistType>
int OLDHNSWIndex_Multi<DataType, DistType>::deleteVector(const labelType label) {
...
template <typename DataType, typename DistType>
int OLDHNSWIndex_Multi<DataType, DistType>::addVector(const void *vector_data,
...
template <typename DataType, typename DistType>
VecSimBatchIterator *
OLDHNSWIndex_Multi<DataType, DistType>::newBatchIterator(const void *queryBlob,
    ...
    return new (this->allocator) OLDHNSWMulti_BatchIterator<DataType, DistType>(
 ...
 */
template <typename DataType, typename DistType>
int OLDHNSWIndex_Multi<DataType, DistType>::markDelete(labelType label) {
 ...
 */
template <typename DataType, typename DistType>
int OLDHNSWIndex_Multi<DataType, DistType>::unmarkDelete(labelType label) {
    auto search = label_lookup_.find(label);
    if (search == label_lookup_.end()) {
        return false;
    }
