...
class OLDHNSWIndex_Single : public OLDHNSWIndex<DataType, DistType> {
private:
    vecsim_stl::unordered_map<labelType, idType> label_lookup_;

#ifdef BUILD_TESTS
#include "VecSim/algorithms/origin_hnsw/hnsw_single_tests_friends.h"
#endif
...
public:
    OLDHNSWIndex_Single(const HNSWParams *params, std::shared_ptr<VecSimAllocator> allocator,
                        size_t random_seed = 100, size_t initial_pool_size = 1)
        : OLDHNSWIndex<DataType, DistType>(params, allocator, random_seed, initial_pool_size),
          label_lookup_(this->max_elements_, allocator) {}
#ifdef BUILD_TESTS
    // Ctor to be used before loading a serialized index. Can be used from v2 and up.
    OLDHNSWIndex_Single(std::ifstream &input, const HNSWParams *params,
                        std::shared_ptr<VecSimAllocator> allocator,
                        Serializer::EncodingVersion version)
        : OLDHNSWIndex<DataType, DistType>(input, params, allocator, version),
          label_lookup_(this->max_elements_, allocator) {}
#endif
    ~OLDHNSWIndex_Single() {}
...
template <typename DataType, typename DistType>
size_t OLDHNSWIndex_Single<DataType, DistType>::indexLabelCount() const {
    return label_lookup_.size();
}

template <typename DataType, typename DistType>
double OLDHNSWIndex_Single<DataType, DistType>::getDistanceFrom(labelType label,
...
template <typename DataType, typename DistType>
void OLDHNSWIndex_Single<DataType, DistType>::replaceIdOfLabel(labelType label, idType new_id,
                                                               idType old_id) {
    label_lookup_[label] = new_id;
}

template <typename DataType, typename DistType>
void OLDHNSWIndex_Single<DataType, DistType>::resizeLabelLookup(size_t new_max_elements) {
...
template <typename DataType, typename DistType>
int OLDHNSWIndex_Single<DataType, DistType>::deleteVector(const labelType label) {
...
template <typename DataType, typename DistType>
int OLDHNSWIndex_Single<DataType, DistType>::addVector(const void *vector_data,
...
template <typename DataType, typename DistType>
VecSimBatchIterator *
OLDHNSWIndex_Single<DataType, DistType>::newBatchIterator(const void *queryBlob,
    ...
    return new (this->allocator) OLDHNSWSingle_BatchIterator<DataType, DistType>(
 ...
 */
template <typename DataType, typename DistType>
int OLDHNSWIndex_Single<DataType, DistType>::markDelete(labelType label) {
 ...
 */
template <typename DataType, typename DistType>
int OLDHNSWIndex_Single<DataType, DistType>::unmarkDelete(labelType label) {
    auto search = label_lookup_.find(label);
    if (search == label_lookup_.end()) {
        return false;
    }
