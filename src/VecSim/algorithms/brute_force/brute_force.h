...
public:
    ...
    inline DataType *getDataByInternalId(idType id) const {
        return (DataType *)((char *)vectors + id * this->dataSize);
    }
    ...
    virtual ~BruteForceIndex() {
        munmap(vectors, this->dataSize * indexCapacity());
        close(vectorsFD);
    }
#ifdef BUILD_TESTS
    /**
...
protected:
    ...
    inline void shrinkByBlock() {
        ...
        size_t old_capacity = indexCapacity(), new_capacity = old_capacity - this->blockSize;
        assert(old_capacity >= this->blockSize);
        idToLabelMapping.resize(new_capacity);
        idToLabelMapping.shrink_to_fit();
        int res = ftruncate(vectorsFD, this->dataSize * new_capacity);
        assert(res == 0);
...
void BruteForceIndex<DataType, DistType>::appendVector(const void *vector_data, labelType label) {
    ...
    memcpy(getDataByInternalId(id), vector_data, this->dataSize);
...
void BruteForceIndex<DataType, DistType>::removeVector(idType id_to_delete) {
    ...
    if (id_to_delete != last_idx) {
        ...
        memcpy(getDataByInternalId(id_to_delete), getDataByInternalId(last_idx), this->dataSize);
    }
...
BruteForceIndex<DataType, DistType>::topKQuery(const void *queryBlob, size_t k,
    ...
    const char *cur_vec = (const char *)this->vectors;
    for (idType curr_id = 0; curr_id < this->count; curr_id++, cur_vec += this->dataSize) {
        if (VECSIM_TIMEOUT(timeoutCtx)) {
            delete TopCandidates;
            return new VecSimQueryReply(this->allocator, VecSim_QueryReply_TimedOut);
        }
        DistType cur_dist = this->distFunc(cur_vec, queryBlob, this->dim);

        // If we have less than k or a better score, insert it.
        if (cur_dist < upperBound || TopCandidates->size() < k) {
            TopCandidates->emplace(cur_dist, getVectorLabel(curr_id));
...
BruteForceIndex<DataType, DistType>::rangeQuery(const void *queryBlob, double radius,
    ...
    for (idType curr_id = 0; curr_id < this->count; curr_id++, cur_vec += this->dataSize) {
        ...
        DistType cur_dist = this->distFunc(cur_vec, queryBlob, this->dim);
        if (cur_dist <= radius_) {
            res_container->emplace(getVectorLabel(curr_id), cur_dist);
        }
    }
