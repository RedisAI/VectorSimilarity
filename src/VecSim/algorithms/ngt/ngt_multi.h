...
#include "ngt.h"
// #include "ngt_multi_batch_iterator.h"
#include "VecSim/utils/updatable_heap.h"

template <typename DataType, typename DistType>
class NGTIndex_Multi : public NGTIndex<DataType, DistType> {
private:
    vecsim_stl::unordered_map<labelType, vecsim_stl::vector<idType>> label_lookup_;

#ifdef BUILD_TESTS
    // friend class HNSWIndexSerializer;
// #include "VecSim/algorithms/ngt/ngt_multi_tests_friends.h"
#endif
...
public:
    NGTIndex_Multi(const NGTParams *params, std::shared_ptr<VecSimAllocator> allocator,
...
template <typename DataType, typename DistType>
size_t NGTIndex_Multi<DataType, DistType>::indexLabelCount() const {
...
template <typename DataType, typename DistType>
void NGTIndex_Multi<DataType, DistType>::resizeLabelLookup(size_t new_max_elements) {
...
template <typename DataType, typename DistType>
int NGTIndex_Multi<DataType, DistType>::deleteVector(const labelType label) {
...
template <typename DataType, typename DistType>
int NGTIndex_Multi<DataType, DistType>::addVector(const void *vector_data, const labelType label) {
