...
void HierarchicalNSW<dist_t>::addPoint(const void *data_point, const labeltype label) {
...

    {
        ...
        if (label_lookup_.find(label) != label_lookup_.end()) {
            throw std::runtime_error(
                "The given label already exit in the index. Consider using updatePoint instead");
        }
...
template <typename dist_t>
priority_queue<pair<dist_t, labeltype>> HierarchicalNSW<dist_t>::searchKnn(const void *query_data,
                                                                           size_t k) const {
