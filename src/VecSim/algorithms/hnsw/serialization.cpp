...
namespace hnswlib {
    ...
    void HNSWIndexSerializer::saveGraph(std::ofstream &output) {
        ...
        output.write(hnsw_index->data_level0_memory_, hnsw_index->cur_element_count * hnsw_index->size_data_per_element_);
        ...
        for (size_t i = 0; i < hnsw_index->cur_element_count; i++) {
            ...
            unsigned int linkListSize =
                    hnsw_index->element_levels_[i] > 0 ? hnsw_index->size_links_per_element_ * hnsw_index->element_levels_[i] : 0;
    ...
    void HNSWIndexSerializer::restoreGraph(std::ifstream &input) {
        ...
        input.read(hnsw_index->data_level0_memory_, hnsw_index->cur_element_count * hnsw_index->size_data_per_element_);
        for (size_t i = 0; i < hnsw_index->cur_element_count; i++) {
            if (hnsw_index->available_ids.find(i) != hnsw_index->available_ids.end()) {
                continue;
            }
        ...
        hnsw_index->visited_nodes_handler = std::unique_ptr<VisitedNodesHandler>(
                new(hnsw_index->allocator) VisitedNodesHandler(hnsw_index->max_elements_, hnsw_index->allocator));
#endif

        // Restore the rest of the graph layers, along with the label and max_level lookups.
        hnsw_index->element_levels_ = vecsim_stl::vector<size_t>(hnsw_index->max_elements_, hnsw_index->allocator);
    ...
    HNSWIndexMetaData HNSWIndexSerializer::checkIntegrity() {
        HNSWIndexMetaData res = {.valid_state = false, .memory_usage = -1,
                .double_connections = HNSW_INVALID_META_DATA, .unidirectional_connections = HNSW_INVALID_META_DATA,
                .min_in_degree = HNSW_INVALID_META_DATA, .max_in_degree = HNSW_INVALID_META_DATA};
