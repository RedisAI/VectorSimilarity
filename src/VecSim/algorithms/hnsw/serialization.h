...
public:
    // Wrap hnsw index.
    explicit HNSWIndexSerializer(std::shared_ptr<hnswlib::HierarchicalNSW<DistType>> hnsw_index);
    ...
    void reset(std::shared_ptr<hnswlib::HierarchicalNSW<DistType>> hnsw_index = nullptr);
};
...
template <typename DistType>
void HNSWIndexSerializer<DistType>::saveIndexFields(std::ofstream &output) {
...
template <typename DistType>
void HNSWIndexSerializer<DistType>::saveGraph(std::ofstream &output) {
...
template <typename DistType>
void HNSWIndexSerializer<DistType>::loadIndex_v1(std::ifstream &input) {
    this->restoreIndexFields(input);
    this->restoreGraph(input);
}

template <typename DistType>
void HNSWIndexSerializer<DistType>::restoreIndexFields(std::ifstream &input) {
...
template <typename DistType>
void HNSWIndexSerializer<DistType>::restoreGraph(std::ifstream &input) {
...
template <typename DistType>
HNSWIndexSerializer<DistType>::HNSWIndexSerializer(
    std::shared_ptr<HierarchicalNSW<DistType>> hnsw_index_) {
    hnsw_index = std::move(hnsw_index_); // Hold a reference of the index.
}

template <typename DistType>
void HNSWIndexSerializer<DistType>::saveIndex(const std::string &location) {
...
template <typename DistType>
void HNSWIndexSerializer<DistType>::loadIndex(const std::string &location) {
...
template <typename DistType>
void HNSWIndexSerializer<DistType>::reset(
    std::shared_ptr<hnswlib::HierarchicalNSW<DistType>> hnsw_index_) {
...
template <typename DistType>
HNSWIndexMetaData HNSWIndexSerializer<DistType>::checkIntegrity() {
