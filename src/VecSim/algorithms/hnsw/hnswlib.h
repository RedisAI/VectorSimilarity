	...
	for (tableint selectedNeighbor : selectedNeighbors) {
        ...
        if (sz_link_list_other < Mcurmax) {
            ...
            for (size_t i = 0; i < removed_links_num; i++) {
                ...
                // incoming edges set.
                if (node_incoming_edges->get_bit(selectedNeighbor)) {
                    node_incoming_edges->set(selectedNeighbor, false);
                } else {
                    neighbour_incoming_edges->set(node_id, true);
                }
            }
        }
    }
...
void HierarchicalNSW<dist_t>::repairConnectionsForDeletion(tableint element_internal_id,
    ...
    for (size_t i = 0; i < removed_links_num; i++) {
        ...
        if (node_incoming_edges->get_bit(neighbour_id)) {
            node_incoming_edges->set(neighbour_id, false);
        } else {
            neighbour_incoming_edges->set(node_id, true);
        }
    }
    ...
    for (size_t i = 0; i < updated_links_num; i++) {
        ...
        if (!neighbour_orig_neighbours_set[node_id]) {
            ...
            if (!bidirectional_edge) {
                node_incoming_edges->set(neighbour_id, true);
            }
        }
    }
}
	...
	void HierarchicalNSW<dist_t>::SwapLastIdWithDeletedId(tableint element_internal_id,
		...
		for (size_t level = 0; level <= last_element_top_level; level++) {
			...
			for (size_t i = 0; i < neighbours_count; i++) {
				...
				if (!bidirectional_edge) {
					auto *neighbour_incoming_edges = getIncomingEdgesPtr(neighbour_id, level);
					neighbour_incoming_edges->set(last_element_internal_id, false);
					neighbour_incoming_edges->set(element_internal_id, true);
				}
			}
...
bool HierarchicalNSW<dist_t>::removePoint(const labeltype label) {
    ...
    for (size_t level = 0; level <= element_top_level; level++) {
        ...
        for (size_t i = 0; i < neighbours_count; i++) {
            ...
            if (!bidirectional_edge) {
                auto *neighbour_incoming_edges = getIncomingEdgesPtr(neighbour_id, level);
                neighbour_incoming_edges->set(element_internal_id, false);
            }
        }
