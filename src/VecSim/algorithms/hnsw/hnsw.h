...
struct LevelData {
    ...
    linkListSize totalIncomingLikns;
    ...
    explicit LevelData(std::shared_ptr<VecSimAllocator> allocator)
        : incomingEdges(new (allocator) vecsim_stl::vector<idType>(allocator)),
          totalIncomingLikns(0), numLinks(0) {}
    ...
    void increaseTotalIncomingEdgesNum() { this->totalIncomingLikns++; }
    void decreaseTotalIncomingEdgesNum() { this->totalIncomingLikns--; }
...
protected:
    ...
    void destroyGraphData(ElementGraphData *em);
...
public:
    ...
    LevelData &getLevelData(ElementGraphData *element, size_t level) const;
    ...
    if (candidate_ids.size() > Mcurmax) {
        ...
        for (auto node_id : not_chosen_candidates) {
            if (neighbour_orig_neighbours_set[node_id]) {
                ...
                auto &node_level_data = getLevelData(node_id, level);
...
void HNSWIndex<DataType, DistType>::insertElementToGraph(idType element_id,
    ...
    size_t levels_without_neighbors = 0;
    for (auto level = static_cast<int>(max_common_level); level >= 0; level--) {
        ...
        if (!top_candidates.empty()) {
            ...
            levels_without_neighbors++;
        }
    }
    if (levels_without_neighbors == max_common_level + 1) {
        std::cout << "Empty candidates set" << std::endl;
    }
}
...
void HNSWIndex<DataType, DistType>::removeAndSwap(idType internalId) {
    ...
    for (size_t level = 0; level <= element->toplevel; level++) {
        ...
        for (size_t i = 0; i < cur_level.getNumLinks(); i++) {
            LevelData &neighbour = getLevelData(cur_level.getLinkAtPos(i), level);
