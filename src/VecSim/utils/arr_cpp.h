...
template <typename T> struct array_hdr_t {
    size_t len;
    size_t cap;
    T buf[];
};
...
template <typename T> array_hdr_t<T> *array_hdr(T *arr) {
...
template <typename T> T *array_new(size_t cap) { return array_new_sz<T>(cap, 0); }

template <typename T> T *array_new_len(size_t cap, size_t len) { return array_new_sz<T>(cap, len); }

template <typename T> T *array_ensure_cap(T *arr, size_t cap) {
...
template <typename T> T *array_grow(T *arr, size_t cap) {
    return array_ensure_cap(arr, ++array_hdr(arr)->len);
}

template <typename T> T *array_append(T *arr, T val) {
    arr = array_grow(arr, ++array_hdr(arr)->len);
    arr[array_hdr(arr)->len - 1] = val;
    return arr;
}

template <typename T> size_t array_len(T *arr) { return arr ? array_hdr(arr)->len : 0; }
