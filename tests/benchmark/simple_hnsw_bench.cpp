...
int main() {
    ...
    std::cout << "Config: " << N_VECTORS << " vectors, " << DIM << " dimensions, M=" << M
              << ", ef_construction=" << EF_CONSTRUCTION << ", ef_runtime=" << EF_RUNTIME << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "Insertion time: " << insert_duration.count() << " ms (" 
              << insert_throughput << " vec/s)" << std::endl;
    std::cout << std::endl;
    ...
    for (size_t i = 0; i < N_QUERIES; ++i) {
        VecSimQueryReply* results = VecSimIndex_TopKQuery(index, queries[i].data(), 10, nullptr, BY_SCORE);
    ...
    for (size_t i = 0; i < N_QUERIES; ++i) {
        VecSimQueryReply* results = VecSimIndex_RangeQuery(index, queries[i].data(), 10.0, nullptr, BY_SCORE);
        VecSimQueryReply_Free(results);
    }
    
    auto range_end = std::chrono::high_resolution_clock::now();
    auto range_duration = std::chrono::duration_cast<std::chrono::microseconds>(range_end - range_start);
    ...
    std::cout << "Range (r=10): avg " << avg_range_time << " µs (" 
              << range_throughput << " queries/s)" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "KNN (k=10):   " << avg_knn_time << " µs (" << knn_throughput << " q/s)" << std::endl;
    std::cout << "Range (r=10): " << avg_range_time << " µs (" << range_throughput << " q/s)" << std::endl;
