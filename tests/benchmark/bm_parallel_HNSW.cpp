...
void run_parallel_search_benchmark() {
    ...
    cout << "Creating an HNSW index of size " << n << " with dim=" << dim << " sequentially"
         << endl;
...
#pragma omp parallel num_threads(n_threads)                                                        \
    shared(n, index, dim, distrib, rng, num_queries, queries, total_res, k, cout,                  \
           q_counter) default(none)
    {
        int myID = omp_get_thread_num();
#pragma omp critical
        cout << "Thread " << myID << " start running queries, index size upon search starts: "
             << VecSimIndex_IndexSize(index) << endl;
        while (true) {
            ...
            auto hnsw_results =
                VecSimIndex_TopKQuery(index, queries[next_val], k, nullptr, BY_SCORE);
...
#pragma omp critical
    ...
    for (size_t q = 0; q < num_queries; q++) {
        ...
        auto bf_results = VecSimIndex_TopKQuery(bf_index, queries[q], k, nullptr, BY_SCORE);
        ...
        if (q % 1000 == 1 && q > 1) {
            std::cout << "Accumulated recall after " << q << " queries is "
...
void run_parallel_indexing_benchmark() {
    ...
    cout << "Creating an HNSW index of size " << n << " with dim=" << dim
         << " with parallel indexing" << endl;
    auto started = std::chrono::high_resolution_clock::now();
#pragma omp parallel num_threads(n_threads)                                                        \
    shared(n, index, dim, distrib, rng, num_queries, queries, k, cout, q_counter) default(none)
    {
        int myID = omp_get_thread_num();
#pragma omp critical
...
#pragma omp critical
    ...
    for (size_t q = 0; q < num_queries; q++) {
        started = std::chrono::high_resolution_clock::now();
        auto hnsw_results = VecSimIndex_TopKQuery(index, queries[q], k, nullptr, BY_SCORE);
        done = std::chrono::high_resolution_clock::now();
...
void run_all_parallel_benchmark() {
    ...
    cout << "Creating an HNSW index of size " << n << " with dim=" << dim
         << " with parallel indexing. After 4 seconds, "
            "we start running queries in parallel"
         << endl;
    auto started = std::chrono::high_resolution_clock::now();
#pragma omp parallel num_threads(n_threads)                                                        \
    shared(n, index, dim, distrib, rng, num_queries, queries, total_res, k, cout,                  \
