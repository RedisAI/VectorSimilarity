...
BENCHMARK_DEFINE_F(BM_VecSimBasics, AddVectorNGT)(benchmark::State &st) {
    // Add a new vector from the test vectors in every iteration.
    size_t iter = 0;
    size_t new_id = VecSimIndex_IndexSize(ngt_index);
    size_t memory_delta = 0;
    for (auto _ : st) {
        memory_delta +=
            VecSimIndex_AddVector(ngt_index, queries[(iter % n_queries)].data(), new_id++);
        iter++;
    }
    st.counters["memory"] = (double)memory_delta / (double)iter;

    // Clean-up.
    size_t new_index_size = VecSimIndex_IndexSize(ngt_index);
    for (size_t id = n_vectors; id < new_index_size; id++) {
        VecSimIndex_DeleteVector(ngt_index, id);
    }
}
...
BENCHMARK_DEFINE_F(BM_VecSimBasics, DeleteVectorNGT)(benchmark::State &st) {
    // Remove a different vector in every execution.
    std::vector<std::vector<float>> blobs;
    size_t id_to_remove = 0;
    double memory_delta = 0;
    ...
    for (auto _ : st) {
        ...
        auto removed_vec = std::vector<float>(dim);
        memcpy(removed_vec.data(),
               reinterpret_cast<NGTIndex<float, float> *>(ngt_index)->getDataByInternalId(
                   id_to_remove),
               dim * sizeof(float));
        blobs.push_back(removed_vec);
        st.ResumeTiming();
        ...
        iter++;
        auto delta = (double)VecSimIndex_DeleteVector(ngt_index, id_to_remove++);
        memory_delta += delta;
    }
    st.counters["memory"] = memory_delta / (double)iter;
    ...
    // Restore index state.
    for (size_t i = 0; i < blobs.size(); i++) {
        VecSimIndex_AddVector(ngt_index, blobs[i].data(), i);
    }
}
...
BENCHMARK_DEFINE_F(BM_VecSimBasics, TopK_NGT)(benchmark::State &st) {
    size_t ef = st.range(0);
    size_t k = st.range(1);
    size_t correct = 0;
    size_t iter = 0;
    auto params = VecSimQueryParams{.ngtRuntimeParams = NGTRuntimeParams{.efRuntime = ef}};
    for (auto _ : st) {
        RunTopK_ANN(st, params, iter, k, correct, ngt_index, bf_index);
        iter++;
    }
    st.counters["Recall"] = (float)correct / (float)(k * iter);
}
...
BENCHMARK_DEFINE_F(BM_VecSimBasics, Range_NGT)(benchmark::State &st) {
    double radius = (1.0f / 100.0f) * (float)st.range(0);
    double epsilon = (1.0f / 1000.0f) * (float)st.range(1);
    size_t iter = 0;
    size_t total_res = 0;
    size_t total_res_bf = 0;
    auto query_params = VecSimQueryParams{.ngtRuntimeParams = NGTRuntimeParams{.epsilon = epsilon}};

    for (auto _ : st) {
        auto ngt_results = VecSimIndex_RangeQuery(ngt_index, queries[iter % n_queries].data(),
                                                  radius, &query_params, BY_ID);
        st.PauseTiming();
        total_res += VecSimQueryResult_Len(ngt_results);
        ...
        VecSimQueryResult_Free(bf_results);
        VecSimQueryResult_Free(ngt_results);
        iter++;
        st.ResumeTiming();
    }
    st.counters["Avg. results number"] = (double)total_res / iter;
...
BENCHMARK_DEFINE_F(BM_VecSimBasics, Memory_NGT)(benchmark::State &st) {
    for (auto _ : st) {
        // Do nothing...
    }
    st.counters["memory"] = (double)VecSimIndex_Info(ngt_index).ngtInfo.memory;
}
...
BENCHMARK_REGISTER_F(BM_VecSimBasics, Memory_NGT)->Iterations(1);

BENCHMARK_REGISTER_F(BM_VecSimBasics, AddVectorHNSW)
    ->Unit(benchmark::kMillisecond)
    ->Iterations((long)BM_VecSimBasics::block_size);

BENCHMARK_REGISTER_F(BM_VecSimBasics, AddVectorNGT)
    ->Unit(benchmark::kMillisecond)
    ->Iterations((long)BM_VecSimBasics::block_size);
...
BENCHMARK_REGISTER_F(BM_VecSimBasics, DeleteVectorNGT)
    ->Unit(benchmark::kMillisecond)
    ->Iterations((long)BM_VecSimBasics::block_size);
...
BENCHMARK_REGISTER_F(BM_VecSimBasics, TopK_NGT)
// {ef_runtime, k} (recall that always ef_runtime >= k)
HNSW_TOP_K_ARGS(10, 10)
HNSW_TOP_K_ARGS(200, 10)
HNSW_TOP_K_ARGS(100, 100)
...
BENCHMARK_REGISTER_F(BM_VecSimBasics, Range_NGT)
// {radius*100, epsilon*1000}
// The actual radius will be the given arg divided by 100, and the actual epsilon values
// will be the given arg divided by 1000.
