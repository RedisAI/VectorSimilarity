...
BENCHMARK_DEFINE_F(BM_VecSimBasics, DeleteVectorBF)(benchmark::State &st) {
    ...
    for (auto _ : st) {
        st.PauseTiming();
        auto removed_vec = std::vector<float>(dim);
        auto *vector_block = reinterpret_cast<BruteForceIndex *>(bf_index)
                                        ->getVectorVectorBlock(id_to_remove);
        size_t index = reinterpret_cast<BruteForceIndex *>(bf_index)->getVectorRelativeIndex(id_to_remove);
