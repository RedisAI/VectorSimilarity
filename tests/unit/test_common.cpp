...
TYPED_TEST(UtilsTests, MinMax_Updatable_Heap) {
    ...
    for (size_t i = 0; i < n_riders; ++i) {
        ...
        p = {priorities[n_riders - 1], riders[0]};
        ASSERT_TRUE(heap.peek_min() == p);
        ASSERT_FALSE(heap.empty());
        p = {priorities[i], riders[n_riders - 1 - i]};
        ASSERT_TRUE(heap.pop_max() == p);
    }
    ...
    ASSERT_TRUE(heap.peek_max() == p);
    p = {priorities[SECOND], 2};
    ASSERT_TRUE(heap.peek_min() == p);
    ...
    for (size_t i = len; i > 0; i--) {
        ...
        ASSERT_EQ(p.first, i == 1 ? priorities[THIRD] : priorities[SECOND]);
    }
    ASSERT_EQ(heap.size(), 0);
    ASSERT_TRUE(heap.empty());
    ASSERT_EQ(heap.begin(), heap.end());
    ...
    for (size_t i = 0; i <= last; i++) {
        heap.insert({priorities[SECOND], i});
    }
    ...
    ASSERT_NE(heap.begin(), heap.end());
    p = {-priorities[THIRD], last};
    ASSERT_EQ(heap.begin()->first, p.first);
    ASSERT_EQ(heap.begin()->second, p.second);
    ASSERT_TRUE(heap.peek_min() == p);
    ASSERT_TRUE(heap.pop_max() == p);
    ASSERT_EQ(heap.size(), 0);
    ASSERT_TRUE(heap.empty());
