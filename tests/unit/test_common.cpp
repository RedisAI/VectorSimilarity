...
// takes this path, so a wrong norm reaches the distance kernels before they run, which no amount of
// exactness in the kernels can repair.
//
// This goes through the public API rather than calling the norm helper, because the kernel tests
// append the norm by hand and so never exercised this at all.
TEST(CommonAPITest, NormalizeUint8LargeDimension) {
    // 33,025 is the last dimension whose worst-case total fits a signed int; the rest are past it.
...
// The same path as seen by an index: add an all-255 vector to a uint8 cosine brute force index at a
// dimension past the old overflow point, and query it with itself. Cosine self-distance must be
// about zero, which it cannot be if the stored or query norm is NaN or wildly wrong.
TEST(CommonAPITest, Uint8CosineSelfDistanceAtLargeDimension) {
    constexpr size_t dim = 65537;
    BFParams params = {
        .type = VecSimType_UINT8, .dim = dim, .metric = VecSimMetric_Cosine, .initialCapacity = 2};
