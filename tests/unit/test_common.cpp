...
TYPED_TEST(CommonIndexTest, DumpHNSWNeighborsDebugEdgeCases) {
    ...
    auto res = VecSimDebug_GetElementNeighborsInHNSWGraph(index, 0, &neighbors_data, &top_level);
    ASSERT_EQ(res, VecSimDebugCommandCode_BadIndex);
    ASSERT_EQ(top_level, HNSW_INVALID_LEVEL);
    ...
    res = VecSimDebug_GetElementNeighborsInHNSWGraph(index, 0, &neighbors_data, &top_level);
    ...
    ASSERT_EQ(top_level, 0);
    ASSERT_EQ(neighbors_data[0][0], 0);
    VecSimDebug_ReleaseElementNeighborsInHNSWGraph(neighbors_data, top_level);

    // Try to get non-existing label again.
    res = VecSimDebug_GetElementNeighborsInHNSWGraph(index, 1, &neighbors_data, &top_level);
    ASSERT_EQ(res, VecSimDebugCommandCode_LabelNotExists);
    ASSERT_EQ(top_level, HNSW_INVALID_LEVEL);
    ASSERT_EQ(neighbors_data, nullptr);
