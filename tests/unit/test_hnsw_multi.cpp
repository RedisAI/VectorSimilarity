...
protected:
    VecSimIndexRef *CreateNewIndex(HNSWParams &params) {
    ...
    HNSWIndex_Multi<data_t, dist_t> *CastToHNSW_Multi(VecSimIndexRef *index) {
        return reinterpret_cast<HNSWIndex_Multi<data_t, dist_t> *>(index->index_ref.get());
    }
};
...
TYPED_TEST(HNSWMultiTest, remove_vector_after_replacing_block) {
    ...
    ASSERT_EQ(index->index_ref->indexLabelCount(), 2);
...
// Add up to capacity.
TYPED_TEST(HNSWMultiTest, resize_and_align_index) {
    size_t dim = 4;
    size_t n = 10;
    size_t bs = 3;
    size_t n_labels = 3;
...
TYPED_TEST(HNSWMultiTest, resize_and_align_index_largeInitialCapacity) {
    size_t dim = 4;
    size_t n = 10;
    size_t bs = 3;
    size_t n_labels = 3;
...
TYPED_TEST(HNSWMultiTest, resize_and_align_index_largerBlockSize) {
    size_t dim = 4;
    size_t n = 4;
    size_t bs = 6;
    size_t n_labels = 3;
...
TYPED_TEST(HNSWMultiTest, hnsw_vector_search_by_id_test) {
    ...
    ASSERT_EQ(VecSimIndex_IndexSize(index), n);
    ASSERT_EQ(index->index_ref->indexLabelCount(), n / per_label);
...
TYPED_TEST(HNSWMultiTest, testCosine) {
    ...
    auto verify_res = [&](size_t id, double score, size_t result_rank) {
        ...
        TEST_DATA_T expected_score = VecSimIndex_GetDistanceFrom(index, id, normalized_query);
...
TYPED_TEST(HNSWMultiTest, markDelete) {
    ...
    unsigned char ep_reminder = index->index_ref->info().hnswInfo.entrypoint % 2;
