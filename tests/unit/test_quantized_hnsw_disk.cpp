...
TEST_F(QuantizedHNSWDiskTest, BasicQuantization) {
    ...
    auto components = CreateQuantizedIndexComponents<float, float>(
        abstractParams.allocator, VecSimMetric_Cosine, dim, false);

    auto *index = new (abstractParams.allocator) HNSWDiskIndex<float, float>(
        &params, abstractParams, components, db, cf);
