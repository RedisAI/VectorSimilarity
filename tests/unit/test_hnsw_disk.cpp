...
TEST_F(HNSWDiskIndexTest, MultiThreadedInsertTest) {
    // Test multi-threaded insertion using mock thread pool (similar to tiered index tests)
    const size_t dim = 64;
    const size_t n = 100;
    ...
    // Create mock thread pool (similar to tiered index tests)
    auto mock_thread_pool = tieredIndexMock();
    mock_thread_pool.thread_pool_size = 4;

    // Create HNSWDiskIndex with job queue parameters
    rocksdb::ColumnFamilyHandle *default_cf = db->DefaultColumnFamily();
    auto *index = new (abstractInitParams.allocator) HNSWDiskIndex<float, float>(
        &params, abstractInitParams, components, db.get(), default_cf, "", 100,
        &mock_thread_pool.jobQ, mock_thread_pool.ctx, tieredIndexMock::submit_callback);

    // Set the index in the mock thread pool context (required for job execution)
    // This is similar to how TieredFactory::NewIndex sets up the index
    mock_thread_pool.ctx->index_strong_ref.reset(index);

    // Set batch threshold high enough so we control when batch is processed
    index->setBatchThreshold(n + 10);

    // Create test vectors and add them
    std::mt19937 rng(42);
    std::vector<std::vector<float>> vectors;
    for (size_t i = 0; i < n; i++) {
        vectors.push_back(createRandomVector(dim, rng));
        normalizeVector(vectors.back());
    }

    // Add all vectors (no batch processing triggered due to high threshold)
    for (size_t i = 0; i < n; i++) {
        int result = index->addVector(vectors[i].data(), i);
        EXPECT_EQ(result, 1);
    }

    // Verify vectors are pending
    EXPECT_EQ(index->indexSize(), n);

    // Launch the BG threads loop
    ...
    // Verify all vectors are in the index
    EXPECT_EQ(index->indexSize(), n);
    EXPECT_EQ(index->indexLabelCount(), n);
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (results->results.size() > 0) {
            successful_queries++;
            // The query vector should be in the results (exact match)
            ...
            EXPECT_TRUE(found_self) << "Vector " << i << " not found in its own query results";
        }
    ...
    EXPECT_GT(successful_queries, n * 0.9) << "Too many queries failed";

    // Note: The mock_thread_pool destructor will clean up the index via index_strong_ref
}
