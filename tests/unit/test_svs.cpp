...
TYPED_TEST(SVSTest, SVSSerializationCurrentVersion) {
    namespace fs = std::filesystem;
    ...
    size_t construction_window_size = 20;
    size_t max_candidate_pool_size = 1024;
    size_t prune_to = 60;

    SVSParams params = {
        .dim = dim,
        .metric = VecSimMetric_L2,
        .multi = false,  // Start with single-label case only
        /* SVS-Vamana specifics */
        .alpha = alpha,
    ...
    VecSimIndex *index = this->CreateNewIndex(params);
    ASSERT_INDEX(index);
    SVSIndexBase *svs_index = this->CastToSVS(index);

    std::vector<std::array<TEST_DATA_T, 4>> v(n);
    std::mt19937 rng;
    rng.seed(47);
    std::uniform_real_distribution<> distrib;
    for (size_t j = 0; j < n; ++j) {
        for (size_t k = 0; k < dim; ++k) {
            v[j][k] = (TEST_DATA_T)distrib(rng);
        }
    }

    std::vector<size_t> ids(n);
    for (size_t j = 0; j < n; ++j) {
        ids[j] = j;  // Use unique IDs
    }
    ...
    fs::path tmp{fs::temp_directory_path()};
    auto subdir = std::string("vecsim_test_100-d4-L2-") + VecSimType_ToString(TypeParam::get_index_type()) +
                  "_single.svs_current_version_" + std::to_string(std::rand());
    auto dir_name = tmp / subdir;
    while (fs::exists(dir_name)) {
        subdir = std::string("vecsim_test_100-d4-L2-") + VecSimType_ToString(TypeParam::get_index_type()) +
                 "_single.svs_current_version_" + std::to_string(std::rand());
        dir_name = tmp / subdir;
    }
    fs::create_directories(dir_name);

    // Save the index with the default version (V0).
    svs_index->saveIndex(dir_name.string());
    ...
    ASSERT_EQ(info.commonInfo.indexLabelCount, n);

    VecSimIndex_Free(index);

    // Load the index from the directory.
    VecSimIndex *serialized_index = this->CreateNewIndex(params);
    ASSERT_INDEX(serialized_index);
    SVSIndexBase *serialized_svs_index = this->CastToSVS(serialized_index);

    serialized_svs_index->loadIndex(dir_name.string());

    // Verify that the index was loaded as expected.
    ASSERT_EQ(serialized_svs_index->getVersion(), SVSSerializer::EncodingVersion::V0);

    VecSimIndexDebugInfo info2 = VecSimIndex_DebugInfo(serialized_index);
    ASSERT_EQ(info2.commonInfo.basicInfo.algo, VecSimAlgo_SVS);
    ASSERT_EQ(info2.commonInfo.basicInfo.isMulti, false);
    ...
    ASSERT_EQ(info2.commonInfo.indexLabelCount, n);
    ...
    ASSERT_EQ(VecSimIndex_IndexSize(serialized_index), n + 1 - 1);
