...
TEST_F(AllocatorTest, test_hnsw_reclaim_memory) {
    ...
    // in.
    ASSERT_LE(initial_memory_size, HNSWIndex::estimateInitialSize(&params, 0) + sizeof(size_t));
    ASSERT_GE(initial_memory_size,
              HNSWIndex::estimateInitialSize(&params, 0) - 2 * vecsimAllocationOverhead);
    ...
    ASSERT_LE(allocator->getAllocationSize(), HNSWIndex::estimateInitialSize(&params, 0) +
                                                  hash_table_memory - vecsimAllocationOverhead);
    ASSERT_GE(allocator->getAllocationSize(), HNSWIndex::estimateInitialSize(&params, 0) +
