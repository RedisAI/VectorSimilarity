...
TYPED_TEST(IndexAllocatorTest, test_hnsw_reclaim_memory) {
    ...
    auto verify_containers_size = [&](size_t expected_size, size_t expected_data_container_blocks,
        ...
        ASSERT_EQ(hnswIndex->vectorBlocks.size(), expected_data_container_blocks);
    ...
    auto &vectors_blocks = hnswIndex->vectorBlocks;
    size_t vectors_blocks_capacity = vectors_blocks.capacity();
    ...
    // Capacity reelects the lower bound, which is the data containers size
    ASSERT_EQ(hnswIndex->indexCapacity(), 0);

    // Add and remove a vector to trigger another resize
    GenerateAndAddVector<TEST_DATA_T>(hnswIndex, d, 0);
    VecSimIndex_DeleteVector(hnswIndex, 0);
